    ...
    if (!isInitialLoad && document.startViewTransition) {
      ...
      transition.finished.finally(() => {
        htmlEl.classList.remove(
            'theme-transitioning-to-dark', 'theme-transitioning-to-light');
      });
    } else {
      changeTheme();
    }
  ...
  function setupLanguage(lang) {
    ...
    if (lang === 'ar') {
      ...
      if (metaDesc)
        metaDesc.setAttribute(
            'content',
            'Official social media and student services directory for Al-Hikma University College. Find news, transportation routes, and student housing.');
    }
  }
  ...
  function initPhysicsCanvas() {
    ...
    class Spark {
      constructor(x, y, isShockwave = false) {
        ...
        const speed = isShockwave ? (Math.random() * 4.5 + 2) :
                                    (Math.random() * 1.5 + 0.4);
        this.vx = Math.cos(angle) * speed;
        this.vy = Math.sin(angle) * speed;
        this.size = Math.random() * 3 + 1.2;
        this.alpha = 1.0;
        this.decay = isShockwave ? (Math.random() * 0.015 + 0.01) :
    ...
    window.addEventListener('mousemove', (e) => {
      if (window.innerWidth < 768)
        return;  // Disable canvas hover tracking/sparks on mobile
    ...
    window.addEventListener('click', (e) => {
      ...
      if (clickX >= 0 && clickX <= canvasRect.width && clickY >= 0 &&
    ...
    class Particle {
      constructor() {
        ...
        if (this.mode === 'equation') {
          this.text =
              academicItems[Math.floor(Math.random() * academicItems.length)];
      ...
      draw() {
        ...
        if (this.mode === 'equation') {
          ctx.font = `${isLight ? '600' : '500'} ${
              this.fontSize}px 'Readex Pro', sans-serif`;
          ...
          if (this.shapeType === 'flask') {
            ...
            ctx.beginPath();
            ctx.ellipse(
                0, 0, this.size * 0.58, this.size * 0.2, Math.PI / 4, 0,
                Math.PI * 2);
            ctx.stroke();
            ctx.beginPath();
            ctx.ellipse(
